...
var ScorePage = React.createClass({
  ...
  render() {
    return (
      <div>
        <ScoreForm submitScore={this.createScore} />
    ...
    return (
      <tbody>
        <tr>
          ...
          <td>{score.title}</td>
          <td>
            <button onClick={this.edit}>Bearbeiten</button>
        ...
        {this.state.editing &&
          <tr>
            <td colSpan="3">
              <ScoreForm score={score} submitScore={this.saveScore} />
            </td>
          </tr>}
      </tbody>
    );
  ...
  render() {
    return (
      <form onSubmit={this.handleSubmit}>
        ...
        <label>ID:</label> <input required type="number" value={this.state.id} onChange={this.handleChange('id')} />
        <label>Titel:</label> <input required type="text" value={this.state.title} onChange={this.handleChange('title')} />
        <input type="submit" />
      </form>
    )
  }
