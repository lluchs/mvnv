...
export const ScorePage = React.createClass({
  ...
  getInitialState() {
    return {
      scores: [],
      showCreateNew: false,
      searchTerm: this.props.location.query.q || '',
  ...
  componentDidMount() {
    ...
    this.dbChanges = db.changes({since: 'now', live: true, include_docs: true}).on('change', (info) => {
  ...
  updateSearch(e) {
    ...
    let query = term ? {q: term} : {}
    this.context.router.replace({query: query})
  },
  ...
  createScore(score) {
    return db.put({
      _id: `score:${score.id.toString().padStart(4, '0')}`,
      ...score
    }).then((result) => {
...
var ScoreForm = React.createClass({
  ...
  handleSubmit(e) {
    ...
    // Don't clutter scores with empty values.
    let addTruthy = (k, v) => v && (score[k] = v)
    addTruthy('composer', this.state.composer)
    addTruthy('year', this.state.year)
    addTruthy('publisher', this.state.publisher)
    this.submitPromise = makeCancelable(this.props.submitScore(score))
    ...
    return (
      <form onSubmit={this.handleSubmit} className="pure-form pure-form-aligned">
        {this.context.loggedIn ||
          <p><em>Änderungen werden nur mit Login synchronisiert.</em></p>}
