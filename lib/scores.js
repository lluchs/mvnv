...
var ScorePage = React.createClass({
  getInitialState() {
    return {scores: [], showCreateNew: false}
  },
  componentDidMount() {
  ...
  render() {
    return (
      <div>
        <p><button onClick={this.createNew} className="pure-button">Neu…</button></p>
        {this.state.showCreateNew &&
          <ScoreForm submitScore={this.createScore} cancel={this.createNew} />}

        <ScoreList scores={this.state.scores} cancel={this.createNew} />
      </div>
    )
  }
})
